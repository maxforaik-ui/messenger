...
function updatePresence(userId: string, delta: 1 | -1) {
  ...
  const online = next > 0;
  const lastSeenAt = lastSeenMap.get(userId) || now;
...
const syncCursorSchema = z.object({ chatId: z.string().min(1), since: z.string().min(1).optional() });
...
function formatMessage(message: any) {
  return {
    ...
    replyTo: message.replyTo ? {
      ...
      body: message.replyTo.deletedAt ? '[deleted]' : message.replyTo.body,
...
io.use((socket, next) => {
  ...
  try {
    const user = jwt.verify(token, socketJwtSecret) as AuthUser;
    socket.data.user = user;
    socket.data.deviceId = socket.handshake.auth?.deviceId || crypto.randomUUID();
    next();
  } catch {
...
io.on('connection', async (socket) => {
  ...
  socket.on('disconnect', async () => {
    ...
    socketHeartbeatTimers.delete(socket.id);
